...
- 📊 **可视化管理** - 查看所有中转路由和落地节点
- 🔀 **端口转发** - 利用singbox进行端口转发，支持TCP、UDP端口转发
...
### 2026.02.24
- **中转脚本新增端口转发**：主脚本菜单选择15，进入中转脚本选择8，即可进入端口转发，利用singbox进行端口转发。可选TCP、UDP进行端口转发，落地节点若使用端口转发，强力建议落地节点为加密节点协议（如：VLESS-Reality、AnyTLS等）。
- **修复了致命BUG**：先前加入解析脚本，为了解析第三方节点链接当中的域名类地址，加入了阿里云和CF的DNS，因为默认的阿里云DNS，会导致某些节点协议在singbox核心下，创建后无法连通。现已改用CF的DNS。
